...
class TikzPrinter : public LatexPrinter {
  public:
    ...
    map<int,pair<double,double>> cordinateIDtoPointforGraphPrinter;
    void setCordinateIDtoPointforGraphPrinter(map<int,pair<double,double>> tmp)
    {
        cordinateIDtoPointforGraphPrinter = tmp;
    }

    map<pair<double, double>, int> cordinateIDforGraphPrinter;
    void setCordinateIDforGraphPrinter(map<pair<double, double>, int> tmp)
    {
        cordinateIDforGraphPrinter = tmp;
    }
    ...
    void autoscale( InputIterator pointsBegin, InputIterator pointsEnd, double documentSize = 15 )
    {
...
public:
    ...
    explicit GraphPrinter(string directory, string filename, string documentType = "article")
    ...
    template< typename RandomAccessIterator, typename PointContainer >
    void drawEdgesforStretchFactor( RandomAccessIterator edgesBegin, RandomAccessIterator edgesEnd, const PointContainer &P, const OptionsList& options = {} ) {
        for( auto e=edgesBegin; e!=edgesEnd; ++e ) {
            Point_2 point1 = Point_2{cordinateIDtoPointforGraphPrinter[e->first].first, cordinateIDtoPointforGraphPrinter[e->first].second};
            Point_2 point2 = Point_2{cordinateIDtoPointforGraphPrinter[e->second].first, cordinateIDtoPointforGraphPrinter[e->second].second};
            //cout<<"edges id "<<e->first<<" "<<e->second<<endl;
            //cout<<"Point1 "<<point1.x()<<" "<<point1.y()<<endl;
            //cout<<"Point2 "<<point2.x()<<" "<<point2.y()<<endl;
            drawEdge(point1, point2, options);
        }
    }
    ...
    template< typename InputIterator >
    void drawVerticesWithID( const InputIterator &pointsStart, const InputIterator &pointsEnd, const OptionsList& options = {}, const OptionsList& borderOptions = {} ) {
        for( auto it=pointsStart; it!=pointsEnd; ++it ) {
            size_t id = cordinateIDforGraphPrinter[make_pair(it->x(),it->y())];
            drawVertexWithLabel(it->x(), it->y(), to_string(id), options, borderOptions);
        }
        m_body.content += "\n";
    }
